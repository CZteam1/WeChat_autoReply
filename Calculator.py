...
def BasicOpe(win):
    ...
    bsin = button(win, Point(2, 10), 1.9, .9, "sin")
    basin = button(win, Point(2, 9), 1.9, .9, "arcsin")  # arcsin
    bfactorial = button(win,Point(2,8),1.9,.9,"n!")
    bcos = button(win, Point(4, 10), 1.9, .9, "cos")  # cos
    bacos = button(win, Point(4.0, 9), 1.9, .9, "arccos")  # arccos
    blog10= button(win, Point(4, 8), 1.9, .9, "lg")
    btan = button(win, Point(6,10), 1.9, .9, "tan")  # tan
    batan = button(win, Point(6.0, 9), 1.9, .9, "arctan")  # arctan
    bradians = button(win, Point(6, 8), 1.9, .9, "rad")
    bDel = button(win, Point(8.5, 10), 2.6, 1, "Del")
    ...
    while not bEsc.clicked(p):
        try:
            ...
            elif bfactorial.clicked(p):
                s = show.getText()
                s = s+ 'factorial('
                show.setText(s)
                show.setSize(20)

            elif blog10.clicked(p):
                s = show.getText()
                s = s + 'log10('
                show.setText(s)
                show.setSize(20)

            elif bradians.clicked(p):
                s = show.getText()
                s = s + 'radians('
                show.setText(s)
                show.setSize(20)
...
def FuncGraph():
    ...
    bPower = button(win, Point(7.8, 8.5), 1.2, .9, "^")
    bSqrt = button(win, Point(9.2, 8.5), 1.2, .9, "sqrt")
    bsin = button(win, Point(2, 10), 1.9, .9, "sin")  # sin
    basin = button(win, Point(2, 9), 1.9, .9, "arcsin")  # arcsin
    blog10 = button(win, Point(2, 8), 1.9, .9, "lg")
    bcos = button(win, Point(4,10), 1.9, .9, "cos")  # cos
    bacos = button(win, Point(4.0, 9), 1.9, .9, "arccos")  # arccos
    blog = button(win, Point(4, 8), 1.9, .9, "ln")
    btan = button(win, Point(6, 10), 1.9, .9, "tan")  # tan
    batan = button(win, Point(6.0, 9), 1.9, .9, "arctan")  # arctan
    bexp = button(win, Point(6, 8), 1.9, .9, "exp")
...
def chfunc():
    win0 = GraphWin('功能选择', 300, 300)
    win0.setBackground('pink')
    win0.setCoords(0.0, 0.0, 40.0, 40.0)

    drawlabel(win0, Point(20.0, 35.0), 'arial', 15, 'bold', 'C&Z计算器')

    bu1 = button(win0, Point(10.0, 25.0), 14.0, 7.0, '')
    drawlabel(win0, Point(10.0, 25.0), 'arial', 12, 'bold', '基础运算')

    bu2 = button(win0, Point(30.0, 25.0), 14.0, 7.0, '')
    drawlabel(win0, Point(30.0, 25.0), 'arial', 12, 'bold', '绘图')

    bu3 = button(win0, Point(10.0, 15.0), 14.0, 7.0, '')
    drawlabel(win0, Point(10.0, 15.0), 'arial', 12, 'bold', '积分')

    bu4 = button(win0, Point(30.0, 15.0), 14.0, 7.0, '')
    drawlabel(win0, Point(30.0, 15.0), 'arial', 12, 'bold', '微分')
